...
NOTE: There is https://github.com/jhipster/generator-jhipster/issues/8373[a duplicate key issue when deploying apps with MongoDB and Elasticsearch to Heroku]. To fix it, modify `src/main/resources/config/application-heroku.yml` to have the following YAML for the `spring.data` key.
...
.Gateway on Heroku
image::chapter5/heroku-welcome.png[Welcome, Java Hipster!, 1200, scaledwidth=100%, align=center]
