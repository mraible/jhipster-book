...
NOTE: There is an https://github.com/jhipster/generator-jhipster/issues/8373[issue when deploying apps with MongoDB and Elasticsearch to Heroku]. To fix it, modify `src/main/resources/config/application-heroku.yml` to have the following YAML for the "spring.data" key. Then run `jhipster heroku` on your store app again. When it prompts you to overwrite `application-heroku.yml`, type `n`.
