...
[source,html]
----
ng-model="goal.user.id" ng-options="user.id as user.id for user in users"
----

To modify it to display `user.login` instead, I simply needed to change the _as_ expression to the following.

[source,html]
----
ng-model="goal.user.id" ng-options="user.id as user.login for user in users"
----

At this point, I added all the generated files to Git; noticing that 54 files had been generated.
