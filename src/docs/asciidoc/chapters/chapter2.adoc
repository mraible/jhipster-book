...
To prove everything was good to go, I ran the Java unit tests using `./gradlew test`. I received a compilation error:
...
I refactored the name of this class from `21PointsApp.java` to `Application.java`. This fixed the compilation error and all
tests passed when running `./gradlew test`. To prove the UI tests worked, I ran `gulp test`.

I started the application with `./gradlew` and found I needed to update the class name in `build.gradle`.

----
:bootRun
Error: Could not find or load main class org.jhipster.health.21PointsApp
----

I its UI integration tests with `gulp itest`. I was surprised to have Firefox crash when Protractor started, so I updated
Firefox to the latest version (47.0.1). This helped, but the tests still failed to run. I figured it'd be good to
make sure Protractor was up-to-date, as well as other dependencies. I installed https://github.com/tjunnone/npm-check-updates[npm-check-updates]
using `npm install -g npm-check-updates`, then ran `ncu`.

----
$ ncu
⸨░░░░░░░░░░░░░░░░░░⸩ ⠇ :
 browser-sync                             2.12.10  →  2.13.0
 del                                        2.2.0  →   2.2.1
 eslint-plugin-angular                      1.0.1  →   1.3.0
 event-stream                               3.3.2  →   3.3.4
 gulp-angular-templatecache                 1.9.0  →   2.0.0
 gulp-eslint                                2.0.0  →   3.0.1
 gulp-flatten                               0.2.0  →   0.3.0
 gulp-imagemin                              3.0.1  →   3.0.2
 gulp-rev                                   7.0.0  →   7.1.0
 gulp-sass                                  2.3.1  →   2.3.2
 gulp-uglify                                1.5.3  →   1.5.4
 jasmine-reporters                          2.1.1  →   2.2.0
 karma                                    0.13.22  →   1.1.1
 karma-coverage                             1.0.0  →   1.1.0
 karma-junit-reporter                       1.0.0  →   1.1.0
 karma-phantomjs-launcher                   1.0.0  →   1.0.1
 protractor                                 3.3.0  →   4.0.0
 protractor-jasmine2-screenshot-reporter    0.3.1  →   0.3.2
 run-sequence                               1.2.1  →   1.2.2
 yargs                                      4.7.1  →   4.8.1

Run ncu with -u to upgrade package.json
----

Everything looked OK, so I ran `ncu -u`, followed by `npm install` and `gulp itest`. Firefox exhibited the same
behavior as before (opened, but didn't connect to the application). To get Protractor working, I had to change a
couple lines in `protractor.conf.js`.
...
Finally, `gulp itest` passed with flying colors.

----
$ gulp itest
[08:18:35] Using gulpfile ~/dev/21-points/gulpfile.js
[08:18:35] Starting 'protractor'...
[08:18:35] W/configParser - pattern ./e2e/entities/*.js did not match any files.
[08:18:35] I/local - Starting selenium standalone server...
[08:18:35] I/launcher - Running 1 instances of WebDriver
[08:18:36] I/local - Selenium standalone server started at http://192.168.1.80:50537/wd/hub
Started
..........


10 specs, 0 failures
Finished in 29.031 seconds
[08:19:07] I/local - Shutting down selenium standalone server.
[08:19:07] I/launcher - 0 instance(s) of WebDriver still running
[08:19:07] I/launcher - firefox #01 passed
[08:19:07] Finished 'protractor' after 32 s
[08:19:07] Starting 'itest'...
[08:19:07] Finished 'itest' after 14 μs
----
...
[master (root-commit) e9880d7] Initial checkin of 21-points application
 362 files changed, 24654 insertions(+)
 ...
...
Writing objects: 100% (488/488), 468.25 KiB | 0 bytes/s, done.
Total 488 (delta 64), reused 0 (delta 0)
