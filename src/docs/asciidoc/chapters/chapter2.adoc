...
image::chapter2/entity-diagram.png[21-Point Health Entity Diagram, 406, scaledwidth="50%", align="center"]
...
image::chapter2/entity-diagram-simple.png[Simple Entity Diagram, 406, scaledwidth="50%", align="center"]
...
After answering the last question, the appropriate files were generated.
...
I ran `grunt test` to ensure my UI tests were good to go, then fired up the app and tried everything out.

* Noticed when creating a goal, user was referenced, rather than name/login. Change Goal.json, regenerate.
