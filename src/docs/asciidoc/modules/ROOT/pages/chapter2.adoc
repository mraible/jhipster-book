...
http://getbootstrap.com/[Bootstrap] is a CSS framework that simplifies the development of web applications. It provides a number of CSS classes and HTML structures that allow you to develop HTML user interfaces that look good. Not only that, but it’s responsive by default, which means it works better (or even best) on a mobile device.
...
The media queries effect screen widths with the given breakpoint or _larger_. For example, `.d-lg-none` sets `display: none` on both `lg`, `xl`, and `xxl` screens.
...
If you'd like to override Bootstrap classes in your project, the easiest thing to do is to put the override rule in a CSS file that comes after Bootstrap's CSS. Or you can modify `src/main/webapp/content/scss/global.scss` directly. Using Sass results in a much more concise authoring environment. Below is the default `vendor.scss` file that JHipster generates. You can see that it imports Bootstrap, and I've added an import for Angular Calendar. Default Bootstrap rules are overridden in the aforementioned `global.scss`.
...
JHipster includes https://ng-bootstrap.github.io/[ng-bootstrap] by default. This library provides Bootstrap 5 components that are powered by Angular instead of jQuery.
...
Internationalization (also called i18n because the word has 18 letters between "`i`" and "`n`") is a first-class citizen in JHipster. Translating an application to another language is easiest if you put the i18n system in place at the beginning of a project. http://www.ngx-translate.com/[ngx-translate] provides directives that make it easy to translate your application into multiple languages. It also provides a service that allows changing the language at runtime.
...
JHipster 7 uses Sass by default. To learn more about structuring your CSS and naming classes, read the great https://smacss.com/[_Scalable and Modular Architecture for CSS_].
...
* `webpack.custom.js` is main configuration file that defines a `development` and a `production` mode. Dev mode enables hot-reloading with Browsersync and desktop notifications. For production, HTML templates are converted to JavaScript, and source maps are created.
