...
[[img-add-points-page]]
.Add Points page
image::chapter1/add-points-page.png[Add Points page, 1095, scaledwidth=90%, align=center]
...
The next business-logic improvement I wanted was to modify list screens, so they'd only show records for current user. Admin users should see all users' data. To facilitate this feature, I modified `PointsResource#getAllPoints()` to have a switch based on the user's role. Rather than showing you the diff of method, here's the whole thing.
...
// todo: check typescript vs javascript
[source,typescript]
...
// todo: check syntax highlighting vs javascript
[source,typescript]
...
if (<user not admin> && points.isPresent() && <user doesn't match current user>) {
    return new ResponseEntity<>("error.http.403", HttpStatus.FORBIDDEN);
}
return ResponseUtil.wrapOrNotFound(points);
----

// todo: create a new pull request in 21-points repo that shows this change
See https://github.com/mraible/21-points-v7/pull/3[21-points#3] for all the changes that I needed to make in resource classes and their tests.
...
After making these changes, I redeployed 21-Points Health to production and used https://developer.chrome.com/docs/lighthouse/[Lighthouse] in Chrome to perform an analysis. You can see the results in the screenshot below.

[[lighthouse-analysis]]
.Lighthouse analysis
image::chapter1/lighthouse-desktop.png[Lighthouse analysis, 1500, scaledwidth=100%, align=center]
