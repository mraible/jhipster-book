...
JHipster ships with support for deploying to Google App Engine, Heroku, and Kubernetes including Microsoft Azure, AWS, and Google Cloud. I used Heroku to deploy my application to the cloud because I'd worked with it before. When you prepare a JHipster application for production, it's recommended to use the pre-configured "`prod`" profile. With Gradle, you can package your application by specifying this profile when building.
...
Opening browser to https://cli-auth.heroku.com/auth/cli/browser/06ab4e96...
Logging in... done
...
// todo: start here once ES on Heroku is fixed
----
$ jhipster heroku
