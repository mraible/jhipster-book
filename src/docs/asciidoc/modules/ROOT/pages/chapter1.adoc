...
I believe TDD (test-driven development) is a great way to write code. However, when developing UIs, I tend to make them work before writing tests. It's usually a very visual activity and, with the aid of Browsersync, there's rarely a delay before you see your changes. I like to write unit tests for my Angular components and directives using https://jestjs.io/[Jest] and I like to write integration tests with https://www.cypress.io/[Cypress].
...
To fix my JHipster app, so it'd recognize this variable, I modified `heroku.gradle` to remove the entire block below for Bansai (that no longer works):
...
[source%autofit] // adding shell only colors a few words
----
$ jhipster ci-cd
...
