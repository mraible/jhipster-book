...
NetBeans has a https://github.com/AlexFalappa/nb-springboot[Spring Boot plugin]. The NetBeans team has been doing a lot of work on web tools support; they have good JavaScript/TypeScript support, and there's a
ifeval::["{media}" != "prepress"]
https://chrome.google.com/webstore/detail/netbeans-connector/hafdlehgocfcodbgjnpecfajgkeejnaa?hl=en[NetBeans Connector]
endif::[]
ifeval::["{media}" == "prepress"]
NetBeans Connector
endif::[]
plugin for Chrome that allows two-way editing in NetBeans and Chrome.
...
Liquibase supports
ifeval::["{media}" != "prepress"]
http://www.liquibase.org/databases.html[most major databases].
endif::[]
ifeval::["{media}" == "prepress"]
most major databases.
endif::[]
If you use MySQL or PostgreSQL, you can use `mvn liquibase:diff` (or `./gradlew generateChangeLog`) to automatically generate a changelog.
